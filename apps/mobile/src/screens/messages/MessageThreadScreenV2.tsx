...
import { Animated, Linking, Modal as RNModal, Pressable, ScrollView, StyleSheet, Text, View } from 'react-native'
...
import { MessageThreadHeaderV2 } from './components/MessageThreadHeaderV2'
import { MessageListV2 } from './components/MessageListV2'
import { MessageDockV2 } from './components/MessageDockV2'
import { useMessageKeyboardDockV2 } from './hooks/useMessageKeyboardDockV2'
import { useMessageThreadControllerV2 } from './hooks/useMessageThreadControllerV2'
import { OptimisticMessage, RenderThreadItem } from './types/message-thread-v2'

type Props = NativeStackScreenProps<MessagesStackParamList, 'MessageThreadV2'>

export function MessageThreadScreenV2({ route, navigation }: Props) {
  const { conversationId, jobContext } = route.params
  const keyboardDock = useMessageKeyboardDockV2()
  const controller = useMessageThreadControllerV2({ conversationId, jobContext })

  return (
    <SafeAreaView style={styles.screen} edges={['top', 'left', 'right']}>
      <View style={styles.content}>
        <MessageThreadHeaderV2
          title={controller.threadTitle}
          isTeamChat={controller.isTeamChat}
          avatarUri={controller.otherUserAvatar}
        ...
        <View style={styles.chatBody}>
          <MessageListV2
            ...
            items={controller.renderItems}
            listReserveTop={keyboardDock.listReserveTop}
            renderItem={({ item }: { item: RenderThreadItem }) => {
              if ('type' in item && item.type === 'DATE') return <DateSeparator date={item.date} />
              const message = item as ChatMessage | OptimisticMessage
              const isMine = message.senderId === controller.userId
              const resolvedReplyTo = message.replyTo || controller.replyFallbackByMessageId[message.id]
              return (
                <MessageBubble
                  message={{ ...(message as ChatMessage), replyTo: resolvedReplyTo } as ChatMessage}
                  isMine={isMine}
                  showSender={controller.isTeamChat && !isMine}
                  onSwipeReply={(swipedMessage) => {
                    controller.setEditingMessage(null)
                    controller.setReplyTo(swipedMessage)
                  }}
                  onReplyPress={(messageId) => {
                    const targetIndex = controller.messageIndexMap.get(messageId)
                    if (typeof targetIndex === 'number') {
                      controller.listRef.current?.scrollToIndex({ index: targetIndex, animated: true, viewPosition: 0.2 })
                    }
                  }}
                  onJobPress={(jobId) => void Linking.openURL(`trimpro://jobs/${jobId}`)}
                  onImagePress={(uri, fileName) => {
                    const attachment = message.attachments?.find((a: any) => a.url === uri)
                    if (attachment?.kind === 'IMAGE') {
                      controller.setViewingMedia({ uri, fileName, kind: 'IMAGE' })
                    } else if (attachment?.kind === 'VIDEO') {
                      controller.setViewingMedia({ uri, fileName, kind: 'VIDEO' })
                    }
                  }}
                  onLongPress={() => controller.openMessageActions(message)}
                />
              )
            }}
          />

          <Animated.View style={[styles.bottomDock, { bottom: keyboardDock.dockBottom }]}>
            <MessageDockV2
              ...
              onOpenMenu={controller.openAttachMenu}
              onVoiceStart={controller.startRecording}
              onVoiceMove={controller.moveRecording}
              onVoiceStop={controller.stopRecording}
              onVoiceCancel={controller.cancelRecording}
              attachments={controller.mediaDrafts}
              onRemoveAttachment={(index) => controller.setMediaDrafts((prev) => prev.filter((_, i) => i !== index))}
              recording={controller.isRecordingUi}
              recordingDurationMs={controller.recordingDurationMs}
              recordingWillCancel={controller.recordingWillCancel}
              replyPreview={
                controller.replyTo
                  ? {
                      senderName:
                        `${controller.replyTo.sender?.firstName || ''} ${controller.replyTo.sender?.lastName || ''}`.trim() ||
                        controller.replyTo.sender?.email ||
                        'Unknown',
                      textPreview:
                        controller.replyTo.text ||
                        controller.replyTo.attachments?.[0]?.fileName ||
                        controller.replyTo.attachments?.[0]?.kind ||
                        '',
                    }
                  : null
              }
              onClearReply={() => controller.setReplyTo(null)}
              sending={controller.sendMutation.isPending}
              disabled={!controller.isOnline}
              onLayoutHeight={keyboardDock.onDockLayout}
            />
          </Animated.View>
        </View>
      </View>

      {controller.viewingMedia ? (
        <MediaViewer
          visible={!!controller.viewingMedia}
          uri={controller.viewingMedia.uri}
          fileName={controller.viewingMedia.fileName}
          kind={controller.viewingMedia.kind}
          onClose={() => controller.setViewingMedia(null)}
        />
      ) : null}

      <ReactNativeModal
        isVisible={controller.showMessageOptions}
        onBackdropPress={controller.closeMessageOptions}
        onBackButtonPress={controller.closeMessageOptions}
      ...
      >
        <View style={styles.messageOptionsSheet}>
          ...
          <Pressable
            style={({ pressed }) => [styles.messageActionRow, pressed && styles.messageActionRowPressed, !controller.canCopyAction && styles.messageActionRowDisabled]}
            onPress={controller.handleCopyAction}
            disabled={!controller.canCopyAction}
          ...
          <Pressable
            style={({ pressed }) => [styles.messageActionRow, pressed && styles.messageActionRowPressed, !controller.canEditAction && styles.messageActionRowDisabled]}
            onPress={controller.handleEditAction}
            disabled={!controller.canEditAction}
            android_ripple={{ color: 'rgba(0,0,0,0.06)' }}
          >
          ...
          <Pressable
            ...
            onPress={controller.handleDeleteAction}
          ...
          <Pressable
            ...
            onPress={controller.closeMessageOptions}
            android_ripple={{ color: 'rgba(0,0,0,0.06)' }}
          >
      ...
      <RNModal visible={controller.showAttachMenu} transparent animationType="slide" onRequestClose={controller.closeAttachMenu}>
        <Pressable style={styles.menuBackdrop} onPress={controller.closeAttachMenu}>
          <Pressable style={styles.menuSheet} onPress={(event) => event.stopPropagation()}>
            <Text style={styles.menuTitle}>Share</Text>
            <ScrollView>
              <Pressable style={styles.menuItem} onPress={() => controller.runAttachMenuAction(controller.pickFromCamera)}>
                <Ionicons name="camera-outline" size={18} color={colors.textPrimary} />
                <Text style={styles.menuItemText}>Camera</Text>
              </Pressable>
              <Pressable style={styles.menuItem} onPress={() => controller.runAttachMenuAction(controller.recordVideoFromCamera)}>
                <Ionicons name="videocam-outline" size={18} color={colors.textPrimary} />
                <Text style={styles.menuItemText}>Record Video</Text>
              </Pressable>
              <Pressable style={styles.menuItem} onPress={() => controller.runAttachMenuAction(controller.pickFromLibrary)}>
                <Ionicons name="images-outline" size={18} color={colors.textPrimary} />
                <Text style={styles.menuItemText}>Photo & Video Library</Text>
              </Pressable>
              <Pressable style={styles.menuItem} onPress={() => controller.runAttachMenuAction(controller.pickDocument)}>
                <Ionicons name="document-outline" size={18} color={colors.textPrimary} />
                <Text style={styles.menuItemText}>Document</Text>
              </Pressable>
              <Pressable style={styles.menuItem} onPress={() => controller.runAttachMenuAction(controller.pickContact)}>
                <Ionicons name="person-outline" size={18} color={colors.textPrimary} />
                <Text style={styles.menuItemText}>Contact</Text>
              </Pressable>
              <Pressable style={styles.menuItem} onPress={() => controller.runAttachMenuAction(controller.shareLocation)}>
...
import { useMessageKeyboardDock } from './useMessageKeyboardDock'
import { colors, spacing, typography } from '../../theme/tokens'
...
export function MessageThreadScreenV2({ route, navigation }: Props) {
  ...
  const { dockBottom, tabBarBaseline, tabBarHeight, keyboardHeight } = useMessageKeyboardDock({
    debugEnabled: SHOW_LAYOUT_DEBUG,
  })
  const debugLayout = useCallback(
    (label: string, payload: Record<string, unknown>) => {
      if (!SHOW_LAYOUT_DEBUG) return
      console.log(`[chat-layout-v2] ${label}`, payload)
    },
    [SHOW_LAYOUT_DEBUG]
  )
    ...
    const optimistic: OptimisticMessage = {
      ...
      replyTo: replyTo
        ? {
            ...
            senderName:
              `${replyTo.sender?.firstName || ''} ${replyTo.sender?.lastName || ''}`.trim() || replyTo.sender?.email || 'Unknown',
  ...
  const listTopReserve = dockHeight + tabBarBaseline
      ...
      <View style={styles.layout}>
        ...
        <View style={styles.messageRegion}>
          <FlatList
            ...
            contentContainerStyle={[styles.listContent, { paddingTop: listTopReserve }]}
          ...
          <Animated.View
            ...
            onLayout={(event) => {
              ...
              debugLayout('dock-layout', {
                dockHeight: nextHeight,
                listTopReserve: nextHeight + tabBarBaseline,
                tabBarHeight,
                tabBarBaseline,
                keyboardHeight,
              })
            ...
            {SHOW_LAYOUT_DEBUG ? (
              <View style={styles.debugBar}>
                <Text style={styles.debugText}>
                  tab:{tabBarBaseline} dock:{dockHeight} key:{keyboardHeight}
                </Text>
              </View>
            ) : null}
