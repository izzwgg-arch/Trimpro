...
import {
  generatePaymentGroupReceiptPdfByToken,
  getPaymentGroupReceiptHtmlByToken,
...
export async function GET(
  ...
  try {
    ...
    if (wantsHtml) {
      const result = await getPaymentGroupReceiptHtmlByToken(token)
      if (!result) {
    ...
    const result = await generatePaymentGroupReceiptPdfByToken(token)
    if (!result) {
      return NextResponse.json({ error: 'Receipt not found or expired' }, { status: 404 })
    }
    ...
    console.error('Public group receipt error:', error)
